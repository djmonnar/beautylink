...
function Toast({ msg, ok, onClose }: { msg: string; ok: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed top-20 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2 px-5 py-3 rounded-xl shadow-lg text-sm text-white"
...
function ServiceFormModal({ service, designers, onClose, onSave }: ServiceFormModalProps) {
  ...
  async function handleSubmit() {
    if (!form.name.trim()) { setNameErr("메뉴명을 입력해주세요."); return; }
    setSaving(true);
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              >
                {(["컷", "펌", "염색", "클리닉", "두피케어", "기타"] as const).map((c) => (
          ...
          {isEdit && (
            <div className="flex items-center justify-between rounded-xl border border-gray-200 px-4 py-3">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 rounded-full transition-colors ${form.active ? "bg-blue-600" : "bg-gray-200"}`}
              >
                <span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition-transform ${form.active ? "translate-x-6" : "translate-x-1"}`} />
              </button>
            </div>
          )}
        </div>
...
function ActiveBadge({
  ...
  if (!canManage) {
    return (
      <span className={`text-xs font-medium px-2 py-1 rounded-full ${service.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}>
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full transition-colors ${service.active ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
      >
        {service.active ? "사용 중" : "비활성"}
        <ChevronDown size={10} />
      </button>
      {open && (
        <div className="absolute left-0 mt-1 w-28 bg-white border border-gray-200 rounded-xl shadow-lg z-20 overflow-hidden">
          {service.active ? (
            <button
              onClick={(e) => { e.stopPropagation(); onToggle(false); setOpen(false); }}
            ...
            <button
              onClick={(e) => { e.stopPropagation(); onToggle(true); setOpen(false); }}
...
export default function ServicesPage() {
  ...
  const canReadServices = true; // all roles
  ...
  const userName = userData?.name ?? authUser?.email ?? "";
  const actingRole: PermissionRole =
    userRole === "owner" ? "원장" : userRole === "manager" ? "매니저" : "디자이너";
  const actingUser = { uid: userId, name: userName, role: actingRole };
  ...
  const loadData = useCallback(async () => {
    ...
    // designers are optional — don't block
    ...
    return (
      <AdminLayout title="시술 메뉴 관리" description="">
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="font-medium text-gray-700">접근 권한이 없습니다.</p>
        </div>
      </AdminLayout>
    );
  }
  ...
  async function handleSave(data: Omit<ServiceMenu, "id">, id?: string) {
    if (id) {
      ...
      // update selectedService if open
  ...
  async function handleToggleActive(service: ServiceMenu, active: boolean) {
    ...
    if (selectedService?.id === service.id) setSelectedService((prev) => prev ? { ...prev, active } : prev);
    const action = active ? "service_reactivated" : "service_deactivated";
    logServiceAccess(salonId, action, service.id, actingUser);
        ...
        <div className="flex-1 space-y-4">
          ...
          {dataLoading ? (
            ...
            <div className="bg-white rounded-xl border border-red-100 shadow-sm flex flex-col items-center justify-center py-16 gap-3">
              ...
              <button onClick={loadData} className="text-sm text-blue-600 underline">다시 시도</button>
            </div>
          ) : (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    {["카테고리", "메뉴명", "가격", "소요시간", "담당 디자이너", "네이버 매칭", "상태", ""].map((h) => (
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-4 py-12 text-center text-sm text-gray-400">
                        {search || category !== "전체" || statusFilter !== "all"
                          ? "검색 결과가 없습니다."
                          : "등록된 시술 메뉴가 없습니다."}
                      </td>
                    </tr>
                    ...
                    filtered.map((s) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          ...
                          {s.description && (
                            <p className="text-xs text-gray-400 mt-0.5 truncate max-w-[160px]">{s.description}</p>
                          )}
                        </td>
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 sticky top-6">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-3 py-1.5 border-b border-gray-50">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${CAT_COLORS[selectedService.category]}`}>
                ...
                <div className="py-1.5">
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {designers.filter((d) => selectedService.assignedDesignerIds.includes(d.id)).length === 0 && (

...
- **작업 단계**: Phase 16b — 네이버예약 제휴 제안 패키지 구축

---

## 최근 작업 요약

### Phase 16b — 네이버예약 제휴 제안 패키지 구축

#### 목적

네이버예약 API 제휴 또는 협력 검토용 제안서 문서 패키지 + 앱 내부 안내 보강

#### 주요 변경 파일

| 파일 | 변경 내용 |
|------|----------|
| `docs/NAVER_PARTNERSHIP_PROPOSAL.md` | **신규** — 제휴 제안서 (문제정의·기대효과·요청사항·파일럿 계획) |
| `docs/NAVER_API_REQUIREMENTS.md` | **신규** — API 요구사항 정의 (필요 데이터·4단계 연동 로드맵·보안 원칙) |
| `docs/PILOT_PLAN.md` | **신규** — 파일럿 운영 계획서 (4주·진주 1~3개 매장·측정 지표) |
| `src/app/page.tsx` | 네이버 섹션 → "연동을 고려한 구조" 4카드 + 5단계 프로세스로 고도화 |
| `src/app/integrations/naver/page.tsx` | 하단 제휴 준비 안내 박스 추가 |
| `docs/DEMO_SCRIPT.md` | 네이버 제휴 검토자용 5분/10분 데모 시나리오 추가 |

#### 신규 문서 요약

**`docs/NAVER_PARTNERSHIP_PROPOSAL.md`**
- 제안 제목: "뷰티샵 전용 CRM '뷰티링크'의 네이버예약 연동 제휴 제안"
- 한 줄 요약, 문제 정의 5종, 뷰티링크 해결책 8가지
- 네이버 입장 기대효과 5종
- 구현 완료 범위 표 (13개 항목 ✅)
- 제휴 요청 사항 5종, 파일럿 제안 (4주·진주·1~3개 매장)

**`docs/NAVER_API_REQUIREMENTS.md`**
- 필요 데이터: 매장·예약·디자이너·시술 메뉴 필드 정의
- 4단계 연동 로드맵: 읽기전용 → 매핑 → 상태동기화 → 양방향
- 보안 원칙 7가지 (최소 권한, 매장단위 승인, 마스킹, accessLog, 서버보관, 클라이언트 미노출)
- 현재 구현된 준비 사항 목록
- 미구현 항목 + 예상 구현 방법

**`docs/PILOT_PLAN.md`**
- 대상: 진주 미용실 1~3곳, 네이버예약 사용, 디자이너 2명 이상
- 7단계 세팅 절차 + 3주 운영 일정
- 정량 지표 7종 (Firestore 자동 집계)
- 정성 설문 8문항
- 파일럿 성공 기준 4개

#### 앱 내부 변경

**`/integrations/naver` 하단 안내 박스 (신규)**
- "이 화면은 네이버예약 공식 API 제휴 또는 연동 검토를 위한 사전 매핑 화면입니다"
- "실제 API 호출은 수행하지 않으며, 제휴 승인 후 연동 범위에 따라 확장됩니다"

**랜딩페이지 네이버 섹션 고도화**
- 제목: "네이버예약 연동을 고려한 구조"
- 설명 문구 업데이트
- 4개 준비 카드 추가 (디자이너매핑·시술매핑·통합준비·API확장)
- 하단: "현재 버전은 실제 네이버예약 API를 호출하지 않습니다" 명시
...
### Phase 16 — 뷰티링크 소개/영업용 패키지 구축

#### 목적

MVP 기능 개발 완료 후 미용실 원장님·네이버예약 제휴 검토자에게 보여줄 소개 페이지, 데모 시나리오, 영업 자료 정리

#### 주요 변경 파일

| 파일 | 변경 내용 |
|------|----------|
| `src/app/page.tsx` | 랜딩페이지 전면 고도화 (Hero·문제·기능·데모계정 섹션) |
| `src/components/layout/AppSidebar.tsx` | 하단에 "가이드" 링크 추가 (→ 랜딩 #demo-accounts) |
| `docs/DEMO_SCRIPT.md` | **신규** — 3분/10분 데모 시나리오 + 역할별 시연 순서 |
| `docs/SALES_ONE_PAGER.md` | **신규** — 영업용 기능 요약 One Pager |
| `docs/PRICING_DRAFT.md` | **신규** — Free/Starter/Pro 가격 정책 초안 |
| `docs/FEATURE_STATUS.md` | 소개/영업용 패키지 섹션 추가, 날짜 최신화 |

#### 랜딩페이지 (`/`) 주요 변경

**Hero 섹션**
- H1: "미용실 예약·고객·디자이너 관리를 한 화면에서"
- Sub: "네이버예약, 전화예약, 방문예약까지 통합 관리하는 뷰티샵 전용 CRM"
- CTA: "데모 시작하기" → `/login` (기존 /dashboard → /login 변경)
- "기능 살펴보기" → `#features` 앵커

**문제 제기 섹션**
- 6가지 불편함: 네이버예약 따로 / 전화예약 메모 따로 / 디자이너 스케줄 따로 / 고객 이력 관리 / 노쇼 관리 / 매출 파악

**핵심 기능 섹션 (9개)**
- 4대 기능 교차 상세 블록: 예약 캘린더·고객 CRM·디자이너+시술·문자알림
- 6개 추가 기능 카드 그리드: 예약 등록·대시보드·보안권한·시술메뉴·네이버연동·개인정보보호

**데모 계정 섹션 (신규)**
- 원장(모든 권한) / 매니저(관리 권한) / 디자이너(준비중) 3종 카드
- 이메일·비밀번호: "별도 전달" 문구 (코드에 직접 노출 없음)

**하단 CTA**
- "뷰티샵 운영을 더 쉽게 관리해보세요" + `/login` 유도
- 보조 버튼: "계정 안내 보기" → `#demo-accounts` 앵커

#### 신규 문서

**`docs/DEMO_SCRIPT.md`**
- 3분 데모: 대시보드 → 캘린더 → 예약 등록 → 고객 → 네이버 매핑
- 10분 데모: 7단계 상세 시나리오 + 멘트 가이드
- 원장님 전용 시연 순서 (강조 포인트 포함)
- 네이버 제휴 검토자 전용 시연 순서
- 모바일 데모 체크포인트
- Q&A 5종

**`docs/SALES_ONE_PAGER.md`**
- 서비스 한 줄 소개 / 타깃 고객 / 해결하는 문제
- 핵심 기능 테이블 (10종, 상태 포함)
- 경쟁 비교표 / 기술 스택 / 다음 로드맵

**`docs/PRICING_DRAFT.md`**
- Free: 무료, 예약 월 50건·디자이너 2명·고객 100명
- Starter: 29,000원/월, 디자이너 5명·무제한 예약
- Pro: 59,000원/월, 실 SMS·알림톡·네이버 연동·무제한
- 기능별 플랜 비교표 / 결제 구현 예상 기술 / 현재 미구현 명시

#### 보안 준수
- 실제 계정 이메일/비밀번호 랜딩페이지에 노출 없음
- 네이버예약 API 호출 없음
- 문자/알림톡 실발송 없음
- 기존 데이터 삭제 없음
